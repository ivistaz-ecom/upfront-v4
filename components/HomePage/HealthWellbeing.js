      ...
      <div className="flex flex-col justify-center items-center container mx-auto ">
        <div>
          ...
          <div className="bg-black text-white text-center p-3 ">
            <h3 className="text-4xl mt-3">Health & Wellbeing</h3>
          ...
          <div className="grid lg:grid-cols-2 md:grid-cols-2 gap-4 justify-center items-center">
            {HealthWellbeingData.sections.map((section, index) => (
              <div key={index} className="flex items-start space-x-4 ">
                ...
                {/* Content */}
                <div className="">
                  <h4 className="font-semibold text-lg w-96">{section.title}</h4>
                  <p className="text-gray-600 text-sm w-96">{section.description}</p>
                </div>
              </div>
            ))}
          </div>
